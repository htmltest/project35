...
// Bourne VideoPlayer (JW) v0.1 11.10.11
String.prototype.trim = function () {
	return this.replace(/^\s*(\S*(\s+\S+)*)\s*$/, "$1");
}

var bVideoPlayer = function (vid) {
	var player,
	flash,
	html5,
	videoElement,
		...
		if (document.all && typeof(window.msPerformance) == 'undefined') {
			...
			for (var i = 0; i < srcs.length; i++) {
				if (srcs[i].parentNode == block || srcs[i].parentNode == videoElement) {
					var src = srcs[i].attributes['src'].nodeValue;
					sources[srcs[i].attributes['type'].nodeValue.replace('video/', '')] = src;
				}
			}
		} else {
			for (var i = 0, len = videoElement.childNodes.length; i < len; i++) {
				var src = videoElement.childNodes[i];
				if (src.tagName === 'SOURCE') {
					var source = src.getAttribute('src');
					sources[src.getAttribute('type').replace('video/', '')] = source;
				}
			}
		}
	}
	function getPlayerHeight() {
		var h = parseInt(videoElement.getAttribute('height') || videoElement.attributes('height').nodeValue);
		h = h + 32;
		return h;
	}
	function getPlayerWidth() {
		return parseInt(videoElement.getAttribute('width') || videoElement.attributes('width').nodeValue);
	}
	function trackEvent(type) {
		if (typeof(analyticsRecord[type]) == 'undefined') {
			analyticsRecord[type] = true;
			if (typeof(window._analyticsTrackEvent) != 'undefined') {
				_analyticsTrackEvent('Video', type, videoFile, null);
			}
		}
	}
	function goFlash() {
	...
	if (classes && typeof(classes) != 'undefined') {
		for (i in classes) {
			if (classes.hasOwnProperty(i)) {
				new_element.className = new_element.className + ' ' + classes[i];
			}
		}
	}
